  ...
  for (ievt = 0; ievt < nChData; ievt++) {
    ...
    /* at this point, we have Gretina4M or 4A data */
      ...
      if (idet < runInfo->nGe) {
        ...
        int slope = trap_fixed(signal, 20, 300, 300);       // baseline slope
      ...
      double e_raw, e_adc, e_ctc, e_lamda;
      ...
      if (chan < 100) {
        e_lamda *= Dets[chan].HGcalib[0];
      } else {
        e_lamda *= Dets[chan-100].LGcalib[0];
      }
      ...
      if (1) {
        ...
        // FWHM of A distribution from series noise is ~ 2 keV for rise = 4
        // aovere -= 4.0/sqrt((float) A_E_RISE) * 2.75 * (1.0 - 1593.0/e_ctc);  // 1593 keV = DE
        aovere -= (PZI.bl_rms[chan] * sqrt(2.0 / (float) A_E_RISE) * 2.75 *
