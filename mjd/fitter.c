 ...
 *    pars[9]: sigma ti use for rms of signal values
 *
 */
...
int fitter(float *pars, float *sig, int tlo, int thi)
{
  ...
  float  diff, chisq=0.0f, chisq1=0.0f;
  int    j, k, l, n, t;
  int    npars = NP, ndf = thi - tlo - NP;
  // float errs[NP]={0};


  if (thi - tlo < 400) return 0;
  if (pars[9] == 0) pars[9] = 4.0;
  ...
  chisq1 /= (float) ndf * pars[9]*pars[9];
