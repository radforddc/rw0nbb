...
int eventprescan(FILE *f_in, FILE *ps_f_out, MJDetInfo *Dets, MJRunInfo *runInfo) {
  ...
  while (fread(head, sizeof(head), 1, f_in) == 1) {
    ...
    /* --------------- Gretina4M digitizer ---------------- */
    if (evlen == 1026) {
      printf("\n -------- ERROR: record length (%d) is standard 1026!\n"
             " -------- Is this uncompressed data??\n", evlen);
      return -1;

    }
    /* ------------ do decompression of signal ------------ */
    ss = (unsigned short *) evtdat + 28;
    siglen = decompress_signal(ss, signal, 2*(evlen - 2) - 28);

    // do final write/copy to output
    //evlen = (head[0] & 0x3ffff);
    evlen = 16 + siglen/2;            // new length of event, in 4-byte words
    ...
    fwrite(signal, 2, siglen, ps_f_out);  // write compressed signal to output file
